...
def main():
    ...
    config = {
        ...
        'blur_kernel': 0, # Blur kernel sampling range (0,)
        'blur_sigma_range': (0.0,0.0), # Blur kernel sigma has to be > 0.1
    ...
    train_transform = TensorAugment(
        ...
        normalize_stats=data_stats  # z-score normalisation
    )
    ...
    start_epoch = 0
    if os.path.exists(f"best_cnn_model_blur_{config['blur_sigma_range']}_{config['model_type']}.pt"):
        print("Resuming from checkpoint...")
        checkpoint = torch.load(f"best_cnn_model_blur_{config['blur_sigma_range']}_{config['model_type']}.pt", map_location=device, weights_only=True)
    ...
    for epoch in range(config['epochs']):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                'val_acc': val_acc,
                'config': config
            }, f"best_cnn_model_blur_{config['blur_sigma_range']}_{config['model_type']}.pt")
    ...
    plot_training_progress(train_loss_hist, train_acc_hist, val_acc_hist, f'cnn_training_progress_blur_{config['blur_sigma_range']}_{config['model_type']}.png')
    
    # Generate feature map visualization
    if len(test_loader) > 0:
        plot_feature_maps(model, test_loader, device, f'cnn_feature_maps_blur_{config['blur_sigma_range']}_{config['model_type']}.png')
