...
class BigViT(nn.Module):
    """Bigger ViT for double descent exploration"""
...
def main():
    ...
    config = {
        ...
        'epochs': 80,
        'patience': 12,
        'k_samples': 150,  # Number of samples to use
    }
    ...
    train_transform = TensorAugment(
        ...
        p_flip=0.5,
        p_rot=0.5,
        noise_std=0, # stop noise tempoerarily
        apply_blur=False
    )
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        T_max=config['epochs'],  # or total number of steps if finer control
        eta_min=1e-6  # minimum LR, optional
    )
    ...
    checkpoint = torch.load("best_vit_model.pt", map_location=device,weights_only=True)
