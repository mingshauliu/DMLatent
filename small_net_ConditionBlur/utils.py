...
class CosmicWebDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            image, sigma = self.transform(image)
        
        return image, sigma, torch.tensor(label, dtype=torch.float32)
...
def evaluate_model(model, data_loader, criterion, device):
    ...
    with torch.no_grad():
        for images, sigmas, labels in data_loader:
...
def extract_conditioned_features(model, dataloader, device):
    model.eval()
    features, labels = [], []

    with torch.no_grad():
        for imgs, sgms, lbls in dataloader:
            imgs = imgs.to(device)
            sgms = sgms.to(device).view(-1, 1).to(dtype=imgs.dtype)
            ...
            # Concatenate sigma
            x = torch.cat([x, sgms], dim=1)  # [B, 257]
