...
class CosmicWebDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mu_cdm = np.load("mu_pk_cdm.npy")
        mu_wdm = np.load("mu_pk_wdm.npy")
            
        pk_raw = radial_power_spectrum(image.cpu())
        pk_log = np.log1p(np.abs(pk_raw))  # Log scale for stability
        
        return image, torch.tensor(label, dtype=torch.float32), torch.tensor(pk_log)
...
# Alternative version if you want to load only specific indices to save memory
def load_dataset_memory_efficient(indices, transform=None, cdm_file='cdm_data.npy', wdm_file='wdm_data.npy'):
    """
    Memory-efficient version that only loads required samples
    ...
    Returns:
        CosmicWebDataset: PyTorch dataset containing only the requested samples
    """
    try:
        # Load full datasets (you might want to use memory mapping for very large files)
        cdm_data = np.load(cdm_file, mmap_mode='r')  # Memory mapped for efficiency
        wdm_data = np.load(wdm_file, mmap_mode='r')
        
        print(f"CDM data shape: {cdm_data.shape}")
        print(f"WDM data shape: {wdm_data.shape}")
        
        # Extract only the needed samples
        max_idx = max(indices)
        cdm_samples = cdm_data[:min(len(cdm_data), max_idx + 1)]
        wdm_samples = wdm_data[:min(len(wdm_data), max_idx + 1)]
        
        # Create dataset
        dataset = CosmicWebDataset(cdm_samples, wdm_samples, indices, transform)
        print(f"Created memory-efficient dataset with {len(dataset)} samples")
...
def radial_power_spectrum(img_tensor):
    ...
    img = img_tensor.cpu().numpy() # Convert to numpy array
    print(np.shape(img))  # Print shape for debugging
    delta = img - np.mean(img)  # Subtract mean to get delta field
...
def compute_class_mean_pk(dataset, num_samples=500):
    """
    Compute mean power spectra for CDM and WDM from a dataset.
    
    Args:
        dataset: an instance of CosmicWebDataset
        num_samples: number of CDM and WDM samples to use for computing the mean
    
    Returns:
        mu_cdm, mu_wdm: mean P(k) vectors for CDM and WDM
    """
    cdm_pk_list = []
    wdm_pk_list = []
    cdm_count = 0
    ...
    for i in range(len(dataset)):
        img, label = dataset[i][:2]  # Ignore pk for now if it returns it
        pk = radial_power_spectrum(img)

        if label == 0.0 and cdm_count < num_samples:
            cdm_pk_list.append(pk)
            cdm_count += 1
        elif label == 1.0 and wdm_count < num_samples:
            wdm_pk_list.append(pk)
            wdm_count += 1

        if cdm_count >= num_samples and wdm_count >= num_samples:
            break

    mu_cdm = np.mean(cdm_pk_list, axis=0)
    mu_wdm = np.mean(wdm_pk_list, axis=0)

    return mu_cdm.astype(np.float32), mu_wdm.astype(np.float32)
...
def evaluate_model(model, data_loader, criterion, device):
    ...
    with torch.no_grad():
        for images, labels, pks in data_loader:
            images, labels, pks = images.to(device), labels.to(device).unsqueeze(1), pks.to(device)
