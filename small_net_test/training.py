...
from models import WDMClassifierDilatedResNet, WDMClassifierTiny, WDMClassifierMedium, WDMClassifierLarge, WDMClassifierHuge
...
def main():
    ...
    elif config['map_type'] == 'star':
        cdm_file='/n/netscratch/iaifi_lab/Lab/msliu/CMD/data/IllustrisTNG/Maps_Mstar_IllustrisTNG_LH_z=0.00.npy'
        if config['data_type'] == 'sid':
            wdm_file='/n/netscratch/iaifi_lab/Lab/msliu/Maps_Mstar_IllustrisTNG_WDM_z=0.00.npy'
        else:
    ...
    train_transform = TensorAugment(
        ...
        normalize=stats  # Normalize images to [0, 1]
    )
    ...
    start_epoch = 0
    if os.path.exists(f"best_cnn_model_blur_{config['map_type']}_{config['data_type']}.pt"):
        print("Resuming from checkpoint...")
        checkpoint = torch.load(f"best_cnn_model_blur_{config['map_type']}_{config['data_type']}.pt", map_location=device, weights_only=True)
    ...
    for epoch in range(config['epochs']):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                'val_acc': val_acc,
                'config': config
            }, f"best_cnn_model_blur_{config['map_type']}_{config['data_type']}.pt")
    ...
    plot_training_progress(train_loss_hist, train_acc_hist, val_acc_hist, f'cnn_training_progress_blur_{config['map_type']}_{config['data_type']}.png')
    
    # Generate feature map visualization
    if len(test_loader) > 0:
        plot_feature_maps(model, test_loader, device, f'cnn_feature_maps_blur_{config['map_type']}_{config['data_type']}.png')
