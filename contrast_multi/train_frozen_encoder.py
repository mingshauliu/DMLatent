...
from frozen_encoder_module import FrozenEncoderContrastiveModel
...
def create_cosmology_focused_config():
    ...
    config = {
        ...
        # FREEZE ENCODER to preserve cosmology knowledge
        'freeze_encoder': True,  # KEY CHANGE: Preserve 80% accurate cosmology features
        ...
        'data_root': '/n/netscratch/iaifi_lab/Lab/msliu/',
    }
    return config

def setup_cosmology_data_paths():
    """Setup data paths - UPDATE THESE TO YOUR ACTUAL PATHS"""
    return {
...
def main():
    ...
    print("=" * 60)
    print("🔬 COSMOLOGY-FOCUSED CONTRASTIVE TRAINING v2")
    print("=" * 60)
    print("GOAL: Improve CDM/WDM separation by PRESERVING pretrained features")
    print("NEW STRATEGY: Freeze encoder (80% accurate), train only projection heads")
    print(f"Strategy: 3-phase training with adaptive weights + frozen encoder")
    ...
    print("  🔒 ENCODER: Frozen to preserve cosmology knowledge")
    print("  🎯 TRAINING: Only projection heads learn new representations")
    print("=" * 60)
    ...
    model = FrozenEncoderContrastiveModel(config)
    ...
    print("  val/encoder_cosmology_accuracy   - Should stay ~0.8 (encoder still works)")
    print("  train/cosmology_specific_loss    - Should decrease steadily")
    print("  train/pretrained_guidance        - Should stay > 0.5 (good alignment)")
    ...
    print("  - Encoder features preserve 80% cosmology accuracy")
    print("  - Only projection heads learn hierarchical representations")
    print("  - Cosmology projector works directly on encoder features")
...
def analyze_cosmology_separation(model_path, data_module):
    ...
    # Load model
    model = FrozenEncoderContrastiveModel.load_from_checkpoint(model_path)
    model.eval()
