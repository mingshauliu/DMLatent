...
def main():
    ...
    config = {
        ...
        # Model type
        'model_type': 'unetbigger',  # Options: 'unet', 'unetsmaller'
        ...
        'max_epochs': 80,
        'patience': 20,
        'dropout': 0.5,  # Dropout rate for the model
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            # Get output and intermediate features
            logits, feats_dict = model.model(x, return_features=True)

            pooled_feat = feats_dict["pooled_features"]  # [B, 272]
        ...
        with torch.no_grad():
            # Call the first conv layer (enc1) directly
            fmap = model.model.enc1(sample_x)  # Output shape: [4, C, H, W]
            fmap = fmap.cpu()

        # Plot first 4 feature maps from the first image
        fig, axes = plt.subplots(1, 4, figsize=(12, 3))
        for i in range(4):
            if i < fmap.shape[1]:  # Prevent index error if fewer than 4 channels
