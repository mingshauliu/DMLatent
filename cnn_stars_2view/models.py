...
class CNNAttentionDualBranch(nn.Module):
    def __init__(self, in_channels=1, num_classes=1, dropout=0.3):
        super().__init__()
        self.features_img = CNNAttentionPooling(in_channels, 128, dropout)  # reuse your block
        self.features_ft = CNNAttentionPooling(in_channels, 128, dropout)
    ...
    def forward_features(self, x_img, x_ft):
        f_img = self.features_img.forward_features(x_img)  # [B, 128]
        f_ft = self.features_ft.forward_features(x_ft)     # [B, 128]
        return torch.cat([f_img, f_ft], dim=1)             # [B, 256]

    
    def forward(self, x_img, x_ft):
        f_img = self.features_img.forward_features(x_img)  # [B, 128]
        f_ft = self.features_ft.forward_features(x_ft)     # [B, 128]
        f = torch.cat([f_img, f_ft], dim=1)                # [B, 256]
        return self.fc(f)
