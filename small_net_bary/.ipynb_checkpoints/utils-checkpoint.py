...
class CosmicWebDataset(Dataset):
    ...
    def __init__(self, cdm_data, wdm_data, indices, bary_cdm=None, bary_wdm=None, subtract_bary=False, transform=None):
        """
        Args:
        ...
        self.bary_cdm = bary_cdm
        self.bary_wdm = bary_wdm
        self.subtract_bary = subtract_bary
        ...
        for idx in indices:
            if idx < len(cdm_data):
                self.total_map.append(('cdm', idx))
                self.labels.append(0.0)  # CDM = 0
            if idx < len(wdm_data):
                self.total_map.append(('wdm', idx))
    ...
    def __getitem__(self, idx):
        data_type, data_idx = self.total_map[idx]
        label = self.labels[idx]
        
        # Get the appropriate sample
        if data_type == 'cdm':
            image_total = self.cdm_data[data_idx]
            if self.bary_cdm is not None and self.subtract_bary:
                image_bary = self.bary_cdm[data_idx]
                image_total = image_total - image_bary

        else:  # wdm
            image_total = self.wdm_data[data_idx]
            if self.bary_wdm is not None and self.subtract_bary:
                image_bary = self.bary_wdm[data_idx]
                image_total = image_total - image_bary
        # Convert to tensor
        image_total = torch.from_numpy(image_total).float()
        
        # Ensure image_total has shape [H, W] (remove extra dimensions if needed)
        if image_total.dim() > 2:
            image_total = image_total.squeeze()
        
        # Apply transforms if provided
        if self.transform:
            image_total = self.transform(image_total)
        
        return image_total, torch.tensor(label, dtype=torch.float32)



def load_dataset(indices, transform=None, cdm_file='cdm_data.npy', wdm_file='wdm_data.npy', bary_file=None):
    """
    ...
    try:
        ...
        if bary_file is not None:    
            print(f"Loading Baryonic data from {bary_file}...")
            bary_cdm, bary_wdm = bary_file.split(',')
            bary_cdm_data = np.load(bary_cdm)
            bary_wdm_data = np.load(bary_wdm)
            print(f"Baryonic data shape: {bary_cdm_data.shape}")
