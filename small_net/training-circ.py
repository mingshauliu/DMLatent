...
from models import WDMClassifierDilatedResNet, WDMClassifierTiny, WDMClassifierMedium, WDMClassifierLarge, WDMClassifierHuge, WDMClassifierLargeCircular
...
def main():
    ...
    config = {
        ...
        'model_type': 'large-circ',  # 'tiny', 'medium', 'large', 'huge'
    ...
    print(f'Normalising with mean: {stats['mean']} and std: {stats['std']}')
    ...
    elif config['model_type'] == 'large-circ':
        model = WDMClassifierLargeCircular(
            num_classes=1,
            dropout=config['dropout']
        ).to(device)
        print("Using WDMClassifierLargeCircular")
