...
from module_newnew import AstroFlowMatchingDataModule, FlowMatchingModel
...
if __name__ == "__main__":
    ...
    for model_name in config['models']:
        ...
        cdm_mass = np.load(f'/n/netscratch/iaifi_lab/Lab/msliu/CMD/data/{model_name}/Maps_Mcdm_{model_name}_{dataset}_z=0.00.npy')
        astro_params = np.loadtxt(f'/n/netscratch/iaifi_lab/Lab/msliu/CMD/data/{model_name}/params_{dataset}_{model_name}.txt')
        star_maps = np.load(f'/n/netscratch/iaifi_lab/Lab/msliu/CMD/data/{model_name}/Maps_Mstar_{model_name}_{dataset}_z=0.00.npy')
        gas_maps = np.load(f'/n/netscratch/iaifi_lab/Lab/msliu/CMD/data/{model_name}/Maps_Mgas_{model_name}_{dataset}_z=0.00.npy')
        T_maps = np.load(f'/n/netscratch/iaifi_lab/Lab/msliu/CMD/data/{model_name}/Maps_T_{model_name}_{dataset}_z=0.00.npy')
        P_maps = np.load(f'/n/netscratch/iaifi_lab/Lab/msliu/CMD/data/{model_name}/Maps_P_{model_name}_{dataset}_z=0.00.npy')
        ...
        if model_name == 'IllustrisTNG':
            astro_params = astro_params[:,[0,1,6,7,8]] # Grab Ωm, σ8, Ωb, h, ns
        elif model_name == 'Astrid':
            astro_params = [np.append(a[[0,1,6]],[0.6711,0.9624]) for a in astro_params] # Astrid SB7 only varies Ωm, σ8, Ωb
        else:
            astro_params = [np.append(a[:2],[0.049,0.6711,0.9624]) for a in astro_params] # LH sets only vary Ωm, σ8
