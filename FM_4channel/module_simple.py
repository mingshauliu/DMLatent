...
from models_simple import UNetScalarField
...
class FlowMatchingModel(pl.LightningModule):
    ...
    def forward(self, x, t, condition, resnet_input):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Use target_maps (which contains star and gas channels) as input to ResNet branch
        resnet_input = target_maps  # Shape: (batch, 2, H, W)
        
        # Predict scalar field
        predicted_field = self(x_t, condition_param, cdm_mass, resnet_input)
    ...
    def validation_step(self, batch, batch_idx):
        # Handle optional model_id
        if len(batch) == 4:
            cdm_mass, target_field, params, model_id = batch
        else:
            cdm_mass, target_field, params = batch
            model_id = None
        ...
        # Use target_field (which contains star and gas channels) as input to ResNet branch
        resnet_input = target_field  # Shape: (batch, 2, H, W)
    ...
    def sample(self, cdm_mass, astro_params, cdm_mass_condition, resnet_input, num_steps=100, method='euler'):
        ...
        with torch.no_grad():
            for i in range(num_steps):
                ...
                field_change = self(x, combined_condition, cdm_mass_condition, resnet_input)
