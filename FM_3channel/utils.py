...
class AstroMapDataset(Dataset):
    def __init__(self, total_mass_maps: np.ndarray, star_maps: np.ndarray, gas_maps: np.ndarray, T_maps: np.ndarray, params: np.ndarray, model_ids: np.ndarray = None, transform=None):
        ...
        self.target_maps = torch.stack([star_tensor, gas_tensor, T_tensor], dim=1)  # Shape: (N, 3, H, W)
